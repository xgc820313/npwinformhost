...
{
	//mPlugin = new nsPluginInstance(aPlugin);
  mPlugin = aPlugin;
  mRefCnt = 0;
}
...
NS_IMETHODIMP nsScriptablePeer::LoadClassUI(const char *aAssemblyLocation, const char *aclassFullName, PRBool *_retval)
{
	try
	{
		(*_retval) = PR_TRUE;
  
		(*_retval) = ClassLoader::Instance->LoadAssemblyUrlLocation( gcnew System::String(aAssemblyLocation));

		ClassLoader::Instance->SetMainUserControlName(gcnew System::String(aclassFullName));

		if (ClassLoader::Instance->MainUserControl == nullptr)
			throw gcnew Exception("Cannot load the user control..");

		if (mPlugin)
			mPlugin->SetUserControl(ClassLoader::Instance->MainUserControl);
		
		//escribir en estatus que el control se cargo correctamente
	...
	catch (Exception^ ex)
	{
		...
		MessageBox::Show(ex->Message +innerExMsg + ex->StackTrace);
		(*_retval) = PR_FALSE;
	}
